...
class M_main extends CI_Model{
	...
	function pendingusulan($table){
		$this->db->select('*');
		$this->db->from($table);
		$this->db->like('status_spt','PENDING');
	  $query = $this->db->get()->result();
		return $query;
		 }
		 function pendingspt($table){
			 $this->db->select('*');
			 $this->db->from('tbl_disposisi');
			 $this->db->like('status_disposisi','PENDING');
			 $query = $this->db->get()->result();
			 return $query;
				}
	...
	function total_usul() {
    return $this->db->get('tbl_spt')->num_rows();
  }
	function total_spt() {
    return $this->db->get('tbl_disposisi')->num_rows();
  }
	function total_kwitansi() {
    return $this->db->get('tbl_rinci')->num_rows();
  }
	function total_sppd() {
    return $this->db->get('tbl_sppd')->num_rows();
  }
}
